...
contract MoodNftTest is Test {
    ...
    function testNftMoodCanFlip() public {
        vm.prank(USER);
        moodNft.mintNft();
        MoodNft.Mood previousMood = moodNft.getMood(0);

        vm.prank(USER);
        moodNft.flipMood(0);

        MoodNft.Mood afterMood = moodNft.getMood(0);

        assertEq(uint256(previousMood), uint256(MoodNft.Mood.HAPPY));
        assertEq(uint256(afterMood), uint256(MoodNft.Mood.SAD));
    }
